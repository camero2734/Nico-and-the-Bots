...
export async function generateScoreCard(member: GuildMember): Promise<Buffer> {
	...
	const totalBetweenLevels = LevelCalculator.pointsToNextLevel(
		LevelCalculator.calculateScore(dbUser.level),
	);
	const remainingBetweenLevels = LevelCalculator.pointsToNextLevel(
		dbUser.score,
	);
	...
	const src =
		Object.values(albumRoles).find((id) => member.roles.cache.get(id)) ||
		albumRoles.SAI;
	...
	if (!avatar_url)
		avatar_url = `https://ui-avatars.com/api/?background=random&name=${member.displayName}`;
	...
	const background = await loadImage(
		`./src/Assets/images/score_cards/${backgroundName}.png`,
	);

	//FIND SHORTEST NAME FOR USER
	let username = member.displayName;
	if (member.user.username.length < username.length)
	...
	while (
		(measuredTextWidth > maxWidth || measuredTextHeight > maxHeight) &&
		checkingSize > 5
	) {
		checkingSize--;
