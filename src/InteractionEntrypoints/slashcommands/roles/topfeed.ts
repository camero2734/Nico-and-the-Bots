...
const command = new SlashCommand({
	description:
		"Gives you notification roles for topfeed channels (when someone posts on SM, DMAORG updates, etc.)",
	options: [
		{
			name: "removeall",
...
command.setHandler(async (ctx) => {
	...
	if (ctx.opts.removeall) {
		...
		return ctx.send({
			embeds: [
				new EmbedBuilder()
					.setDescription("All your roles have been removed.")
					.toJSON(),
			],
		});
	}
...
const genChoiceId = command.addInteractionListener(
	"topfeedChoose",
	[],
	async (ctx) => {
		...
		const hasRoles = [...member.roles.cache.values()].filter((r) =>
			allRoles.includes(r.id),
		);
