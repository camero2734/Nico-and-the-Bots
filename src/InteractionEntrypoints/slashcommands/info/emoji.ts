...
command.setHandler(async (ctx) => {
	...
	for (const emojiMention of emojis) {
		...
		const author =
			emoji.author?.id && (await ctx.guild.members.fetch(emoji.author.id));
		...
		if (author)
			embed.addFields([
				{ name: "Creator", value: userMention(author.user.id) },
			]);
	...
	if (embeds.length === 0)
		throw new CommandError("A valid emoji was not provided.");
