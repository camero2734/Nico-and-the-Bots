module.exports = {
    execute: async function (msg) {
        ...
        console.log(chalk.red("RESETTING TEAMS"));
        try {
            let numOnTeam = 5;
            let winningTeams = await selectWinningTeam();
            if (!Array.isArray(winningTeams)) return console.log(winningTeams, /NO ARRAY/);
            ...
            await writeJsonFile("json/teams.json", teamsJSON);
            for (let team of finalTeams) {
                for (let member of team.team) {
                    ...
                    try {
                        let dm = await msg.guild.members.get(member).createDM();
                        dm.send(`Monthly teams have been added! You are now a member of Team ${teamNum}! Your Team Captain is <@${team.captain}>, and your fellow team members are:\n${teamStr}\n\nYour team captain will be choosing a team name and icon. If you have ideas, suggest it to them!`);
                    } catch(e) {
                        console.log(e, /TEAMMEMERR/);
                    }
            ...
            for (let team of finalTeams) {
                ...
                try {
                    let dm = await msg.guild.members.get(team.captain).createDM();
                    dm.send(`Monthly teams have been added! You are now the captain of Team ${teamNum}! Your team members are:\n${teamStr}\n\nYou will be choosing a team name and icon. To do this, simply say \`!setupteam\` in #commands for a guided process. Make sure to ask your team for ideas regarding the name and icon first!`);
                } catch(e) {
                    console.log(e, /TEAMCAPERR/);
                }
                await new Promise(done => setTimeout(done, 2000));
            }
