...
const command = new SlashCommand({
	...
	options: [
		...
		},
		{
			name: "url",
			description:
				"A direct link to the image, video, or audio file. Max 50MB.",
			required: true,
			type: ApplicationCommandOptionType.String,
		},
	],
...
command.setHandler(async (ctx) => {
	...
	const chan = ctx.channel.guild.channels.cache.get(
		channelIDs.mulberrystreet,
	) as TextChannel;
	...
	const lastSubmitted = dbUser.lastCreationUpload
		? dbUser.lastCreationUpload.getTime()
		: 0;
	...
	if (Date.now() - lastSubmitted < MS_24_HOURS && ctx.user.id !== userIDs.me) {
		...
		throw new CommandError(
			`You've already submitted! You can submit again ${timestamp}.`,
		);
	}
	...
	if (!fileType)
		throw new CommandError("An error occurred while parsing your file");

	if (
		!["audio", "video", "image"].some((mime) => fileType.mime.startsWith(mime))
	) {
		console.log(fileType);
		throw new CommandError(
			"Invalid file type. Must be a url to an image, video, or audio file.",
		);
	}
