...
command.setHandler(async (ctx) => {
	...
	if (!roleIDs || roleIDs.length === 0) {
		throw new CommandError(
			`You don't have any color roles! Visit <#${channelIDs.shop}> to learn how to get them.`,
		);
	}
...
const roleSelectedId = command.addInteractionListener(
	"roleSelected",
	["originalUserId"],
	async (ctx, args) => {
		...
		if (!roleIds.includes(role.id)) {
			throw new CommandError(
				`You don't own this color role (or it is not a color role)`,
			);
		}
		...
		if (currentlyEquippedRoles.includes(role.id)) {
			const embed = new EmbedBuilder()
				.setTitle("Success!")
				.setDescription("Removed your color role");
