...
export class TimedInteractionListener<IDs extends Readonly<string[]>> {
	...
	constructor(
		private ctx:
			| typeof SlashCommand.GenericContextType
			| typeof ContextMenu.GenericContextType
			| Message,
		names: IDs,
	) {
		this.customIDs = names.map((n) => {
			const hash = F.hash(names.join(",") + ctx.id + ctx.member?.user.id).slice(
				0,
				10,
			);
			return `eph&${n}${hash}`;
	...
	async wait(
		...
		const promises = this.customIDs.map((customID) => {
			return new Promise<[string, MessageComponentInteraction] | []>(
				(resolve) => {
