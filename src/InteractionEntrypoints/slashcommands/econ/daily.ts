...
command.setHandler(async (ctx) => {
	await ctx.reply({
		embeds: [
			new EmbedBuilder({
				description: "Connecting to Daily Electronic Message Archive...",
				color: 0xff0000,
			}),
		],
	});
	...
	const isWeeklyBonus =
		consecutiveDailyCount > 0 && consecutiveDailyCount % 7 === 0;
	...
	const creditsToGive =
		(isCommon ? 300 : 200) * (hasDoubleCredits ? 2 : 1) +
		(isWeeklyBonus ? 2000 : 0);
	...
	const background = await loadImage(
		`./src/Assets/images/daily_cards/${backgroundName}.png`,
	);
	...
	const embed = new EmbedBuilder()
		...
		.setFooter({
			text: "Have an idea for another server tip? Submit it with /submit suggestion",
		})
		...
		.addFields([
			{
				name: "Total tokens",
				value: `You have **${newTokens}** token${F.plural(newTokens)}.`,
			},
		])
