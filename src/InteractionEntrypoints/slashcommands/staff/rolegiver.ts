...
command.setHandler(async (ctx) => {
	...
	const actionRow = new ActionRowBuilder<ButtonBuilder>().setComponents([
		new ButtonBuilder()
			...
			.setCustomId(
				genActionId({ roleId: roleObj.id, action: `${ActionTypes.Give}` }),
			)
			.setEmoji({
				name: "😎",
			}),
		new ButtonBuilder()
			...
			.setCustomId(
				genActionId({ roleId: roleObj.id, action: `${ActionTypes.Remove}` }),
			)
			.setEmoji({
				name: "😔",
			}),
	...
	const embed = new EmbedBuilder()
		...
		.setDescription(
			`By clicking the buttons below, you can get/remove the ${roleObj.name} role.`,
		)
