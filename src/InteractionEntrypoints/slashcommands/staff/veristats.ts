...
command.setHandler(async (ctx) => {
	...
	const partitionByCorrectness = R.partition(
		(a: VerifiedQuizAnswer): boolean => a.answer === 0,
	);
	...
	const embed = new EmbedBuilder().setTitle(
		`${F.titleCase(ctx.opts.stat)} Verified Quiz Questions`,
	);
	...
	for (const stat of topSortedStats) {
		...
		embed.addFields([
			{
				name: questionName,
				value: `${progress} [${stat.correct}/${stat.total}]`,
			},
		]);
	}
