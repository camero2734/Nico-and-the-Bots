...
command.setHandler(async (ctx) => {
	...
	if (!fmUsername) {
		...
		const instructions = `**To set up your fm account:**
            ...
            \`4.\` The \`/fm now\` command should now work properly and display what you are currently listening to!`.replace(
			/^ +/gm,
			"",
		);
		await ctx.send({
			embeds: [
				new EmbedBuilder({ description: instructions, color: Colors.Red }),
			],
		});
		return;
	}

	const existingUserItem = await prisma.userLastFM.findUnique({
		where: { username: fmUsername },
	});
	...
	if (existingUserItem) {
		...
		throw new CommandError(
			`Someone already connected the last.FM user \`${fmUsername}\``,
		);
	}
	...
	if (lastTrack?.name) {
		trackEmbed = new EmbedBuilder()
			...
			.setTitle(
				"This is your most recently scrobbled song. Does this look correct?",
			)
	...
	if (buttonPressed !== yesId) {
		const failEmbed = new EmbedBuilder({
			description: "Setting FM username cancelled.",
		});
	...
	await ctx.send({
		embeds: [
			new EmbedBuilder({
				description: `Succesfully updated your FM username to \`${fmUsername}\``,
			}),
		],
		components: [],
	});
