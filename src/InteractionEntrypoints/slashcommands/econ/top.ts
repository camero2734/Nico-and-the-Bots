...
const command = new SlashCommand({
	...
	options: [
		...
		},
		{
			name: "timeperiod",
			description:
				"The time period over which to display statistics. Defaults to 1 month.",
			required: false,
			type: ApplicationCommandOptionType.Integer,
...
command.setHandler(async (ctx) => {
	...
	const memberScores =
		timeperiod === 0
			? await getAlltimeScores(ctx, pageNum)
			: await getMemberScores(ctx, pageNum, timeperiod);

	if (memberScores.length === 0)
		throw new CommandError("This page doesn't exist!");
	...
	await ctx.send({
		...
		files: [
			{
				name: `top-over${timeperiod || 0}-page${pageNum}.png`,
				attachment: buffer,
			},
		],
	});
...
async function getMemberScores(
	...
	return await Promise.all(
		paginatedUsers.map(async (u) => ({
			member: await ctx.member.guild.members
				.fetch(u.userId.toSnowflake())
				.catch(() => undefined),
			score: u.score,
		})),
	);
...
async function generateImage(
	...
	const img = await loadImage(
		"./src/Assets/images/leaderboard_background2.png",
	);
	...
	for (let i = 0; i < userScores.length; i++) {
		...
		if (
			member &&
			textInfo.width > 200 &&
			member.user.username.length < displayedName.length
		) {
