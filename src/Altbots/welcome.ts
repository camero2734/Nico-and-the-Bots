...
export class SacarverBot {
	...
	async beginWelcomingMembers(): Promise<void> {
		...
		this.client.on("interaction", (interaction) => {
			...
			if (interaction.customId === ANNOUNCEMENTS_ID)
				return this.giveAnnouncementsRole(interaction);
		});

		this.client.on("guildMemberUpdate", (...data) =>
			this.handleMembershipScreening(...data),
		);
	}
	...
	static async generateWelcomeImage({
		...
		const name = displayName
			...
			.trim()
			.toLowerCase();
		const fontSize = F.canvasFitText(ctx, canvas, name, fontFamily, {
			maxWidth: 600,
			maxFontSize: 45,
		});
	...
	async handleMembershipScreening(
