...
async function attachInfo(song) {
    return new Promise(resolve => {
        if (typeof song === 'undefined' || !song || song === "") resolve(false)
        else {
            console.log(song.url, /SONGURL/)
            ytdl.getInfo(song.url, (err, info) => {
                if (err) {
                    console.log(err, /YTDL_ERR/)
                    resolve(false)
                }
                ...
                colorLog("Downloading...", 'pink')
                let length = `${padString(Math.floor(info.length_seconds / 60))}:${padString(Math.floor(info.length_seconds - 60 * Math.floor(info.length_seconds / 60)))}`
                ...
                song.info = {title: info.title, author: {name: info.author.name, avatar: info.author.avatar, url: info.author.user_url}, length: length, thumbnail: thumb}
                
                var urlEnd = encodeUrl(info.title.replace(/\(.*?\)|\[.*?\]/g, ""))
                snekfetch.get("https://api.genius.com/search?q=" + urlEnd, { headers: { Authorization: `Bearer HfRtuFR2txhAmu5c4EhBLoqHZWF9qUn5pgaDdYDky_tUHOLHYdBuJL3v28ugBUNV` }, }).then((r) => {
...
async function playNext() {
    ...
    voiceChannel.join().then(async (vc) => {
        await sendInfo(song);
        var stream = ytdl(song.url, { quality: 'highestaudio' });
