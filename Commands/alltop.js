module.exports = {
    execute: async function (msg) {
        ...
        //Some initialization 
		let rows = await sql.all(`SELECT DISTINCT * FROM scores2 ORDER BY points DESC`)
        ...
        for (let row of rows) {
            if (bot.users.get(row.userId)) {
                ...
                uName = "invalid_user"
                sql.get(`DELETE FROM scores2 WHERE userId = "${row.userId}"`)
            }
            ...
            if ((rownum >= lownum) && (rownum <= highnum)) {
                ...
                ctx.fillRect(20, (startY-5) + 46.3 * diff, 460, 1.15 * 46.3);
                ...
                if (msg.guild.members.get(row.userId)) {
                    let r = await snekfetch.get(msg.guild.members.get(row.userId).user.displayAvatarURL);
                ...
                //Row num
                ctx.fillText(rownum, 60, (startY+21) + 46.1 * diff)
