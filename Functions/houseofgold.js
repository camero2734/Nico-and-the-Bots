module.exports = async function (msg, Discord, sql) {
    ...
    try {
        if (msg.author.bot) { }
            ...
            try {
                ...
                if (!row || !row.xp || row.xp < cost) return dm.send("Not enough credits!");
                let newXP = row.xp - cost;
                sql.run(`UPDATE daily SET xp=${newXP} WHERE userId="${msg.author.id}"`);
