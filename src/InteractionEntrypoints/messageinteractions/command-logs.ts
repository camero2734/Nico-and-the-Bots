...
const GenStaffDiscussId = msgInt.addInteractionListener(
	"discussEmbedStaff",
	args,
	async (ctx, args) => {
...
EntrypointEvents.on("slashCommandFinished", async ({ entrypoint, ctx }) => {
	...
	const embed = new EmbedBuilder()
		...
		.addFields([
			{ name: "Used", value: F.discordTimestamp(new Date(), "relative") },
		]);

	const staffCommandLogChan = (await member.guild.channels.fetch(
		channelIDs.logs.staffCommands,
	)) as TextChannel;
