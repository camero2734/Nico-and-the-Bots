...
const getChannelName = (ctx: TextBasedChannel | Interaction): string => {
	...
	const interactionType =
		F.keys(InteractionType).find(
			(k) => InteractionType[k as keyof typeof InteractionType] === ctx.type,
		) || "Unknown Interaction";
...
export const ErrorHandler = async (
	...
	const errorDelta = receivedInteractionAt
		? Date.now() - receivedInteractionAt.getTime()
		: null;
	...
	if (errorChannel && !(e instanceof CommandError)) {
		const embed = new EmbedBuilder()
			...
			.setColor("DarkRed")
			.setDescription(
				`An error occurred in ${ctx.constructor.name}.\nError ID: ${errorId}`,
			)
			.addFields({
				name: "Channel",
