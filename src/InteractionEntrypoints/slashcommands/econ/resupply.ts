...
const command = new SlashCommand({
	description:
		"Using a daily token, search one of the Bishop's districts for supplies (credits, roles, etc.)",
	options: [],
});
...
command.setHandler(async (ctx) => {
	...
	const options = districts.map((d, idx) => {
		const bishopName = d.bishop;
		const prefix = userBishop?.name === bishopName ? "📍 " : " ";
		const homeDistrict =
			userBishop?.name === bishopName ? " (Your District)" : "";
		return new StringSelectMenuOptionBuilder()
			.setLabel(`${prefix}DST. ${bishopName.toUpperCase()}`)
			.setDescription(
				`Search ${F.capitalize(bishopName)}'s district${homeDistrict}. ${d.difficulty}.`,
			)
	...
	const buttonActionRow = new ActionRowBuilder<ButtonBuilder>().setComponents([
		new ButtonBuilder()
			.setLabel("View Supply List")
			.setCustomId(genButtonId({}))
...
const genSelectId = command.addInteractionListener(
	"banditosBishopsSelect",
	["matchingBishop"],
	async (ctx, args) => {
		...
		if (!dbUser.dailyBox || !tokens || tokens < 1) {
			const embed = new EmbedBuilder()
				.setDescription(
					"You don't have any tokens! Use the `/econ daily` command to get some.",
				)
				.setThumbnail("attachment://file.gif");
			await ctx.editReply({
				embeds: [embed],
...
const genButtonId = command.addInteractionListener(
	"banditosBishopsButton",
	[],
	async (ctx) => {
		...
		for (let i = 0; i < districts.length; i++) {
			...
			// Prize types
			const prizes = [creditsPrize, ...district.getAllPrizes()].sort(
		...
		for (const [item, description] of Object.entries(ItemDescriptions)) {
			embed.addFields([
				{
					name: `What is a ${item.toLowerCase()}?`,
					value: description,
					inline: true,
				},
			]);
...
async function memberWon(
	...
	await prisma.user.update({
		where: { id: member.id },
		data: {
			credits: dbUserWithBox.credits,
			dailyBox: { update: { steals, blocks } },
		},
	});
