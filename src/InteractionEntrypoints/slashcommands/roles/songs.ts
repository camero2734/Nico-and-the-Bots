...
command.setHandler(async (ctx) => {
	...
	const orderedRoleIds = await withCache(
		"songs-role-id-cache",
		async () => {
	...
	if (!roleIDs.includes(role)) {
		throw new CommandError(
			`You don't own this song role (or it is not a song role)`,
		);
	}
	...
	if (currentlyEquippedRoles.includes(role)) {
		const embed = new EmbedBuilder()
			.setTitle("Success!")
			.setDescription("Removed your song role");
		return ctx.editReply({ embeds: [embed] });
	}
	...
	const embed = new EmbedBuilder()
		.setTitle("Success!")
		.setDescription(`You now have the <@&${role}> song role!`);
	return ctx.editReply({ embeds: [embed] });
});
