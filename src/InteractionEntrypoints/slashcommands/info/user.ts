...
command.setHandler(async (ctx) => {
	...
	// Fetch some info
	const dbUser = await queries.findOrCreateUser(userID, {
		golds: true,
		dailyBox: true,
	});
	...
	const embed = new EmbedBuilder()
		...
		.addFields([
			{ name: "Account created on", value: `${member.user.createdAt}` },
		])
		.addFields([{ name: "Originally joined on", value: `${dbUser.joinedAt}` }])
		.addFields([
			{ name: "Last joined on", value: `${member.joinedAt || new Date()}` },
		])
		.addFields([{ name: "Golds", value: `${golds}`, inline: true }])
		.addFields([
			{ name: "Daily count", value: `${dbUser.dailyBox?.dailyCount || 0}` },
		])
		.setFooter({
