...
const constructHistory = (id: string, rounds: number): [string, SongBattleHistory] => [id, { rounds, eliminations: 0 }];
...
describe("findFirstUnmatchedSongs", () => {
  test("returns first two songs if they haven't been matched up yet", () => {
    const sorted = [constructHistory("song1", 1), constructHistory("song2", 1), constructHistory("song3", 1)];
  ...
  test("chooses minimum rounds if multiple songs are unmatched", () => {
    ...
    const previousBattlesRaw: Poll[] = [constructPoll("song1", "song2"), constructPoll("song1", "song3")];
  ...
  test("throws an error if all songs have been matched up", () => {
    ...
    expect(() => findFirstUnmatchedSongs(sorted, previousBattlesRaw)).toThrow("All songs have been matched up");
  });
});
