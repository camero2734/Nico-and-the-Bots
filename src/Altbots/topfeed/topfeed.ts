...
class TopfeedBot {
	...
	async #createWatchers(): Promise<void> {
		...
		this.websites = [
			...
			new SiteWatcher(
				"http://dmaorg.info/found/103_37/clancy.html",
				"DMAORG 103.37 Page",
				["HTML"],
			),
			new SiteWatcher(
				"http://dmaorg.info/found/103_37/Violation_Code_DMA-8325.mp4",
				"DMAORG Violation MP4",
				["HEADERS"],
			),
		];
		...
		this.youtubes = [
			new YoutubeWatcher(
				"twentyonepilots",
				channelIDs.topfeed.band,
				roles.topfeed.selectable.band,
			),
			new YoutubeWatcher(
				"slushieguys",
				channelIDs.topfeed.tyler,
				roles.topfeed.selectable.tyler,
			),
		];
	}
	...
	async #checkGroup<U>(watchers: Watcher<U>[]): Promise<void> {
		...
		for (const watcher of watchers) {
			if (watcher.type !== watchersType) {
				throw new Error(
					"checkGroup must be called with an array of the same types of watchers",
				);
			}

			const chan = (await this.guild.channels.fetch(
				watcher.channel,
			)) as TextChannel;
			...
			for (const post of allMsgs) {
				...
				const embed =
					mainMsg.embeds?.[0] && EmbedBuilder.from(mainMsg.embeds?.[0]);

				const partialTitle =
					`${embed?.data?.description?.substring(0, 20)}...` ||
					`${watchersType} post`;
				const threadTitle = `${partialTitle} - Media Content`
					.replaceAll("\n", " ")
					.trim();
