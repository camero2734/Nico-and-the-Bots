...
command.setHandler(async (ctx) => {
	...
	const rawPermissions = (<unknown>(
		channelPermissionsBackup.permissions
	)) as OverwriteResolvable[];
	// const permissions = rawPermissions.map((p) => PermissionOverwrites.resolve(<unknown>p as OverwriteData, ctx.guild));

	await ctx.channel.permissionOverwrites.set(
		rawPermissions,
		`Lockdown end by ${ctx.user.id}`,
	);

	await prisma.channelPermissionsBackup.delete({
		where: { channelId: ctx.channel.id },
	});
	...
	const lockEndEmbed = new EmbedBuilder()
		...
		.setDescription(
			"The lockdown in this channel has ended. Normal channel permissions have been restored.",
		);
