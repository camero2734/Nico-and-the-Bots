...
async function handleGold(
	...
	const originalMember = isAdditionalGold
		? await ctxMember.guild?.members.fetch(originalUserId)
		: msg.member;
	...
	if (originalMember.id === ctx.user.id)
		throw new CommandError("You cannot give gold to yourself");

	const previousGold = await prisma.gold.findFirst({
		where: { houseOfGoldMessageUrl: msg.url, fromUserId: ctx.user.id },
	});
	if (previousGold)
		throw new CommandError("You already gave this message gold!");
	...
	const timedListener = new TimedInteractionListener(ctx, <const>[
		"goldCtxYes",
		"goldCtxNo",
	]);
	...
	const chan = (await msg.guild?.channels.fetch(
		channelIDs.houseofgold,
	)) as TextChannel;
	...
	const idx =
		goldEmbed.data.fields?.findIndex((f) => f.name === NOT_CERTIFIED_FIELD) ||
		-1;
	...
	const goldMessage = await prisma.$transaction(
		async (tx) => {
			...
			if (user.credits < 0)
				throw new CommandError("You don't have enough credits!");

			const payload = { embeds: [goldEmbed], components: [goldActionRow] };
			const m = isAdditionalGold
...
new Cron("12 * * * *", async () => {
	...
	const receivedGoldRecentlySet = new Set(
		receivedGoldRecently.map((g) => g.toUserId),
	);
