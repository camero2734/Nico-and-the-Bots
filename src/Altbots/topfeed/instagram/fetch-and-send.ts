...
async function initializeInstagram() {
	...
	await ig.simulate
		.preLoginFlow()
		.catch((error) => console.log("Pre-login flow error:", error?.message));
	const loggedInUser = await ig.account.login(
		secrets.apis.instagram.username,
		secrets.apis.instagram.password,
	);
	void loggedInUser;

	await ig.simulate
		.postLoginFlow()
		.catch((error) => console.log("Post-login flow error:", error?.message));
	initialized = true;
}
...
function extractMedia(item: TimelineFeedResponseMedia_or_ad): InstagramMedia[] {
	...
	for (const mediaItem of carouselMedia) {
		if (
			...
			const bestVideo = mediaItem.video_versions.sort(
				(a, b) => b.width - a.width,
			)[0];
...
export async function instaPostToComponents(
	post: FormattedInstagramPost,
	roleId: string,
) {
	// Compose author line
...
export async function fetchInstagram(source: "scheduled" | "random") {
	...
	await testChan
		.send(`[${source}] Fetching recent IG posts`)
		.catch(console.error);
	...
	try {
		...
		const message =
			error instanceof Error
				? error.message
				: "Unknown error during Instagram initialization";
		console.error(error);
		await testChan.send(
			`${userMention(userIDs.me)} Error initializing Instagram: ${message}`,
		);
		return;
	}
	...
	try {
		...
		const message =
			error instanceof Error
				? error.message
				: "Unknown error during Instagram feed fetch";
		console.error(error);
		await testChan.send(
			`${userMention(userIDs.me)} Error fetching Instagram feed: ${message}`,
		);
	}
}
...
async function sendInstagramPost(post: FormattedInstagramPost) {
	...
	if (
		!usernamesToWatch.includes(post.author as (typeof usernamesToWatch)[number])
	) {
		console.log(
			`Skipping post from ${post.author} as they are not in the watchlist.`,
		);
		return;
	}
	...
	if (existing) {
		console.log(
			`IG post ${post.code} from ${post.author} already exists in the database.`,
		);
	...
	if (addDays(new Date(post.postedAt), 1) < new Date()) {
		console.log(
			`Skipping IG post ${post.code} from ${post.author} as it is old.`,
		);
		await testChan
			.send(`Skipping IG post ${post.url} from ${post.author} as it is old.`)
			.catch(console.error);
		return;
	}
	...
	await testChan
		.send(`Processing IG post ${post.url} from ${post.author}`)
		.catch(console.error);

	const { roleId, channelId } =
		usernameData[post.author as (typeof usernamesToWatch)[number]];
	void channelId;
