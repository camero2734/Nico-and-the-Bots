...
export async function uploadImageToCloudflareStorage(fileName: string, buffer: Buffer, expiresInSeconds = 30) {
    // Check if the file already exists
    const existing = await CloudflareR2.send(new GetObjectCommand({ Bucket: BUCKET, Key: fileName })).catch(() => null);
    ...
    const url = await getSignedUrl(CloudflareR2, new GetObjectCommand({ Bucket: BUCKET, Key: fileName }), { expiresIn: expiresInSeconds });
