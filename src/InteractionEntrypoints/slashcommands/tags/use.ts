...
const canSend = (ctx: typeof command.ContextType): boolean => {
	...
	if (rolesCache.has(roles.deatheaters) || rolesCache.has(roles.staff))
		return true;
...
command.setHandler(async (ctx) => {
	...
	const tagAuthor =
		(await ctx.guild?.members
			.fetch(tag.userId.toSnowflake())
			.catch(() => null)) ||
		(await ctx.guild.members.fetch(ctx.client.user?.id || ""));
	if (!tagAuthor)
		throw new CommandError("The user that created this tag no longer exists");
...
async function sendSuggestionList(
