module.exports = async function(reaction, user, sql) {
    ...
    return new Promise(resolve => {
        ...
        if (msg.channel.id === chans.houseofgold) {
            ...
            (async function() {
                let emoji1 = 0;
                let emoji2 = (await msg.reactions.get("⬇").fetchUsers()).array().length;
        ...
        if (reaction.emoji.id === '389216023141941249') {
            ...
            function sendGold() {
                let id = user.id
                sql.get(`SELECT * FROM daily WHERE userId ="${id}"`).then(row => {
                    if (!row) return;
                    if (row.xp < cost) {
                        reaction.remove(user)
                        reaction.message.channel.send('`Giving gold costs ' + cost + 'credits!`')
                        return;
                    } else {
                        myFunctions.credits(user.lastMessage, -cost)
                        sendtheembed()
                    }
                })
                async function sendtheembed() {
