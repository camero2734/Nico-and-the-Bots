...
const genId = ctxMenu.addInteractionListener(
	"reportMessage",
	["channelId", "messageId"],
	async (ctx, args) => {
		...
		if (!msg)
			throw new CommandError(
				"This message appears to have already been deleted.",
			);
		if (!msgMember)
			throw new CommandError("Could not find member who sent this message.");

		// Ensure this user hasn't reported before
		const userPreviousReport = await prisma.userMessageReport.findUnique({
			where: {
				messageUrl_reportedByUserId: {
					messageUrl: msg.url,
					reportedByUserId: ctx.user.id,
				},
			},
		});
		if (userPreviousReport)
			throw new CommandError("You already reported this message!");
		...
		if (priorReports.length > 0) {
			...
			if (!channelId || !messageId)
				throw new Error("Invalid message url stored");

			const staffChan = (await ctx.guild.channels.fetch(
				channelId,
			)) as TextChannel;
			...
			const btn = actionRow.components.find((c) =>
				c.customId?.startsWith(NULL_CUSTOM_ID_PREFIX),
			);
		...
		await ctx.editReply({
			embeds: [
				new EmbedBuilder({ description: "Your report has been submitted." }),
			],
			components: [],
		});
	},
