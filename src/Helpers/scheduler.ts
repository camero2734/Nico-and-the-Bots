...
import {
	NUM_DAYS_FOR_CERTIFICATION,
	NUM_GOLDS_FOR_CERTIFICATION,
} from "../InteractionEntrypoints/contextmenus/gold";
...
export default async function (client: Client): Promise<void> {
	...
	const doc = new GoogleSpreadsheet(
		"1M63thXZZLKUc-3Y0IZmCLRYK2BsaFbAs_0P1xSeVRd0",
	);
...
async function checkMemberRoles(guild: Guild): Promise<void> {
	...
	const membersToRemoveNew = allMembers.filter((mem) => {
		if (!mem.roles.cache.has(roles.new) || !mem.joinedAt || mem.pending)
			return false;
...
async function checkVCRoles(guild: Guild): Promise<void> {
	...
	const membersWithVcRoleArr = allMembers.filter((m) =>
		m.roles.cache.has(roles.vc),
	);
	const membersWithVcRole = new Collection(
		membersWithVcRoleArr.map((m) => [m.id, m]),
	);
...
async function checkFBApplication(
	guild: Guild,
	...
	for (const row of rows) {
		...
		const keys = Object.keys(row).filter(
			(k) => !k.startsWith("_") && k !== ApplicationIdKey,
		);
