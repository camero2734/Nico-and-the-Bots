...
const command = new SlashCommand({
	description:
		"Creates (or edits) a command that sends a short snippet of text",
	options: [
		{
			name: "name",
...
command.setHandler(async (ctx) => {
	...
	if (existingTag?.userId) {
		if (existingTag.userId.toSnowflake() !== ctx.user.id)
			throw new CommandError("This tag already exists");
	...
	const actionRow = new ActionRowBuilder<ButtonBuilder>().setComponents([
		new ButtonBuilder()
			...
			.setCustomId(
				generateYesID({ name: ctx.opts.name, textLookup: `${textLookup.id}` }),
			),
	]);
...
const generateYesID = command.addInteractionListener(
	"tcYes",
	["name", "textLookup"],
	async (ctx, args) => {
		await ctx.deferReply({ ephemeral: true });
		...
		if (!userCredits || userCredits.score < TAG_COST)
