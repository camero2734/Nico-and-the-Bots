...
command.setHandler(async (ctx) => {
  ...
  if (ctx.member.id !== userIDs.me) throw new CommandError("Under construction");
  ...
  const questionParts = [];
  let pages = Math.ceil(VerifiedQuizConsts.NUM_QUESTIONS / VerifiedQuizConsts.MAX_NUMBER_OF_QUESTIONS_PER_PAGE);
  for (let i = 0; i < pages; i++) {
    const start = i * VerifiedQuizConsts.MAX_NUMBER_OF_QUESTIONS_PER_PAGE;
    const end = start + VerifiedQuizConsts.MAX_NUMBER_OF_QUESTIONS_PER_PAGE;
    questionParts.push(QuestionIDEncoder.encode(questionList.slice(start, end)));
  }

  const actionRow = new ActionRowBuilder<ButtonBuilder>().setComponents([
    new ButtonBuilder()
      .setLabel("Decoding").setEmoji("🕵️")
      .setStyle(ButtonStyle.Success).setCustomId(genModalId({ seed36 })),
    ...questionParts.map((qs, idx) => new ButtonBuilder()
      .setLabel(`Quiz ${idx + 1}`)
      .setDisabled(false) // TODO: Should be disabled until part 1 is done
      .setStyle(ButtonStyle.Secondary).setCustomId(genOpenQuizPart({ num: `${idx + 1}`, questions: qs }))),
    new ButtonBuilder()
      .setLabel("Cancel").setEmoji("❌")
      .setStyle(ButtonStyle.Danger).setCustomId(genCancelId({})),
  ]);

  await ctx.editReply({ embeds: [initialEmbed], components: [actionRow] });
});

const genCancelId = command.addInteractionListener("verifcancel2", [], async (ctx) => {
...
const genModalId = command.addInteractionListener("verifmodal2", ["seed36"], async (ctx, args) => {
  if (!ctx.isButton()) return;
...
const genModalSubmitId = command.addInteractionListener("verifmodaldone2", ["seed36"], async (ctx, args) => {
  ...
  if (!correct) {
    ...
    const embed = new EmbedBuilder()
      .setColor(0xff8888)
      .setDescription(`You failed the verified theories quiz.\n\nYou may apply again in ${hours} hours.`);
    await ctx.editReply({
      embeds: [embed],
      components: [],
    });
...
const genOpenQuizPart = command.addInteractionListener("verifquizpart", ["num", "questions"], async (ctx, args) => {
  if (!ctx.isButton()) return;

  const quizPart = +args.num;
  const questions = QuestionIDEncoder.decode(args.questions);

  const modal = new ModalBuilder();

  modal.setTitle(`Verified Quiz Part ${quizPart}`)
    .setCustomId(Bun.randomUUIDv7()) // TODO
    .addComponents(
      questions.map(q => {
        const select = new StringSelectMenuBuilder();
        select.setCustomId(Bun.randomUUIDv7()) // TODO
        select.setOptions(
          F.shuffle(
        ...
        return {
          id: VerifiedQuizConsts.LABEL_COMPONENT_ID,
          label: q.question,
