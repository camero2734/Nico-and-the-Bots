...
import {
	FB_DELAY_DAYS,
	genApplicationLink,
	getActiveFirebreathersApplication,
} from "./_consts";
...
command.setHandler(async (ctx) => {
	...
	const activeApplication = await getActiveFirebreathersApplication(
		ctx.user.id,
	);
...
export async function sendToStaff(
	...
	try {
		const fbApplicationChannel = (await guild.channels.fetch(
			channelIDs.deapplications,
		)) as TextChannel;
		...
		for (const [name, value] of Object.entries(data)) {
			embed.addFields([
				{ name: name, value: value?.substring(0, 1000) || "*Nothing*" },
			]);
		}
...
const genModalId = command.addInteractionListener(
	...
	async (ctx, args) => {
		...
		const modal = new ModalBuilder()
			...
			.setCustomId(
				genId({ applicationId: args.applicationId, type: action.toString() }),
			);
		...
		const titleInput = new ActionRowBuilder<TextInputBuilder>().setComponents(
			new TextInputBuilder()
				...
				.setPlaceholder(
					`Why was the application ${verbPast}? This is optional and will be sent to the user.`,
				)
...
const genId = command.addInteractionListener(
	"staffFBAppRes",
	["applicationId", "type"],
	async (ctx, args) => {
		...
		if (!application)
			throw new CommandError("This application no longer exists");

		const member = await ctx.guild.members.fetch(application.userId);
		if (!member)
			throw new CommandError("This member appears to have left the server");
		...
		if (action === ActionTypes.Accept) {
			...
			embed.setDescription(
				`You are officially a Firebreather! You may now access <#${channelIDs.fairlylocals}>`,
			);
			...
			embed.setDescription(
				`Unfortunately, your application for FB was denied. You may reapply ${timestamp}`,
			);
		...
		if (thread) {
			await thread.setArchived(
				true,
				"Decision was made, thread no longer necessary",
			);
