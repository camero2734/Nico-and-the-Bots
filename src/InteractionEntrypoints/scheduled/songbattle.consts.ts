	...
	// },
	{
		...
		songs: [
			...
			// { name: "The Craving (single version)", image: "https://i.scdn.co/image/ab67616d00001e02d1e9c8027e794228dc35ad26", yt: "https://youtu.be/H3OiQEOcrA8" },
			{
				...
				image:
					"https://i.scdn.co/image/ab67616d00001e020fde79bfa5e23cb9cbdcd142",
			...
			},
			{
				name: "Heathens",
				image:
					"https://i.scdn.co/image/ab67616d00001e022ca3ba8f334ca5a5f0312efb",
			...
			},
			{
				name: "Doubt (demo)",
				image:
					"https://i.scdn.co/image/ab67616d00001e029b0aa15c3f5e17fa8281aec7",
...
export function fromSongId(id: string): { song: SongContender; album: Album } {
	...
	if (!song)
		throw new CommandError(`Song not found: ${songName} in album ${albumName}`);
...
export async function calculateHistory() {
	...
	for (const battle of previousBattlesRaw) {
		...
		if (!song1 || !song2)
			throw new CommandError(
				`Song not found in history: ${battle.options[0]} or ${battle.options[1]}`,
			);
	...
	const eligibleSongs = Array.from(histories.entries()).filter(
		([_, h]) => h.eliminations < NUMBER_OF_ELIMINATIONS,
	);
	const sorted = F.shuffle(eligibleSongs).sort(
		(a, b) => a[1].rounds - b[1].rounds,
	);
...
export function findFirstUnmatchedSongs(
	...
	for (const [song1Id, history] of sorted) {
		...
		const previousBattles = previousBattlesRaw.filter((b) =>
			b.options.includes(song1Id),
		);
...
export function getTotalMatches(
	history: Awaited<ReturnType<typeof calculateHistory>>,
) {
	const { histories, numTies, fewestEliminations } = history;
	return (
		NUMBER_OF_ELIMINATIONS * (histories.size - 1) + numTies + fewestEliminations
	);
}

export function determineNextMatchup(
	history: Awaited<ReturnType<typeof calculateHistory>>,
): {
	song1: SongContender;
	song2: SongContender;
	...
	const { song1Id, song2Id } = findFirstUnmatchedSongs(
		sorted,
		previousBattlesRaw,
	);
...
export async function createResultsChart(pollId: number) {
	...
	for (const vote of poll.votes) {
		...
		const diffMinutes =
			(voteTime.getTime() - chartStart.getTime()) / (1000 * 60);
