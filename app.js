...
async function chooseKey() {
    ...
    let found = false;
    for (let key of keys) {
        console.log(`${chalk.green("Trying key:")} ${chalk.bgGreen.black(key)}`);
    ...
    if (!found) {
        console.log(chalk.bgRed.white("All NSFW keys have run out."))
...
Discord.chans = chans;
let cookie = loadJsonFile.sync("json/cookies.json");
let profiles = loadJsonFile.sync("json/profiles.json");
let tags = loadJsonFile.sync("json/tags.json");
let boostEmojiJSON = loadJsonFile.sync("json/boostemoji.json");
let autoreactJSON = loadJsonFile.sync("json/autoreact.json");
let quizJSON = loadJsonFile.sync('json/demaquiz.json');
let waitTimes = loadJsonFile.sync('json/waittimes.json');

//Class instantiation/initiation
const bot = new Discord.Client({ disabledEvents: ["TYPING_START"], autoReconnect: true, max_message_cache: 0, fetchAllMembers: true });
...
const reaction_events = { MESSAGE_REACTION_ADD: "messageReactionAdd", MESSAGE_REACTION_REMOVE: "messageReactionRemove" };
bot.on("raw", async event => {
    if (reaction_events.hasOwnProperty(event.t)) {
        ...
        bot.emit(reaction_events[event.t], reaction, user);
    }
...
bot.on("message", async message => {
    ...
    if (!message.member.roles.get("330877657132564480") && message.channel.id === "470434533884297216" && message.attachments.size === 0) return message.delete();

    // Cache images
    let cacheChannel = message.guild.channels.get(chans.spoilerimages);
    for (let [key, value] of message.attachments.entries()) {
        try {
            let file = await (await nodefetch(value.url)).buffer();
            let att = new Discord.Attachment(file, value.filename);
...
bot.on("messageDelete", async message => {
    ...
    let embed = new Discord.RichEmbed()
        .setAuthor(`${message.member.displayName} (${message.author.id})`, message.author.displayAvatarURL)
        .setColor(color)
        .setTitle("❌ Messaged Deleted")
        .setDescription(message.content)
        .addField("Channel", `#${message.channel.name}`, true)
        .addField("Message ID", message.id)
        .setFooter("Original message sent")
        .setTimestamp(message.createdAt);

    let deleteLog = message.guild.channels.get(chans.deletelog);
    if (message.attachments.size === 1) {
        let a = message.attachments.first();
        if (!a.cached) {
            embed.addField("Caching failed", `The file was not cached`);
            await deleteLog.send(embed);
        } else {
            let extension = a.filename.split(".").pop();
            let filetype = mime.getType(extension);
            let att = new Discord.Attachment(a.cached, a.filename);
            if (filetype.startsWith("image")) {
                embed.attachFile(att).setImage(`attachment://${a.filename}`);
                await deleteLog.send(embed);
            } else {
                await deleteLog.send(embed);
                await deleteLog.send(new Discord.RichEmbed().setColor(color).setTitle(`Attachment 1/1 - \`${a.filename}\``));
                await deleteLog.send(att);
            }
        }
    } else {
        await deleteLog.send(embed);

        for (let [i, a] of Object.entries(message.attachments.array())) {
            await new Promise(next => setTimeout(next, 1000));
            let attembed = new Discord.RichEmbed().setColor(color).setTitle(`Attachment ${~~i + 1}/${message.attachments.size} - \`${a.filename}\``);
            if (!a.cached) {
                attembed.setDescription(`The file was not cached`);
                await deleteLog.send(attembed);
                continue;
            }
            let extension = a.filename.split(".").pop();
            let filetype = mime.getType(extension);
            let att = new Discord.Attachment(a.cached, a.filename);
            if (filetype.startsWith("image")) {
                attembed.attachFile(att).setImage(`attachment://${a.filename}`);
                await deleteLog.send(attembed);
            } else {
                await deleteLog.send(attembed);
                await deleteLog.send(att);
...
bot.on("messageReactionAdd", async (reaction, user) => {
    ...
    handleReaction(reaction, user).then(async (r) => {
        ...
        if (type === "strike") {
            ...
            let m;
            if (embed.file) m = await msg.guild.channels.get(chans.houseofgold).send({ embed: embed.embed, files: [embed.file] });
...
bot.on("channelPinsUpdate", async (channel, time) => {
    if (channel.id === chans.creations && doplease) {
        let data = await channel.fetchPinnedMessages();
        let array = data.array();
        let firstPin = array[0];
        await new Promise(next => setTimeout(next, 1000));
        firstPin.unpin();
        let picturesc = firstPin.attachments;
        let pictures = picturesc.array();
        let url = pictures[0].url;

        let r = await snekfetch.get(url);
        let embed = new Discord.RichEmbed();
        embed.addField("\u200b", `[Post Link](https://discordapp.com/channels/269657133673349120/${firstPin.channel.id}/${firstPin.id})`);
        console.log(`https://discordapp.com/channels/269657133673349120/${firstPin.channel.id}/${firstPin.id}`);
        embed.setAuthor(firstPin.member.displayName, firstPin.author.avatarURL);
        embed.setImage(url);
        await channel.guild.channels.get(chans.bestcreations).send({ embed: embed });
    }
});
...
//             let strikeJSON = await fs.readFileAsync("json/strikes.json");
...
//             fs.writeFileQueued("json/strikes.json", strikeJSON);
//         }
//     });
// }
