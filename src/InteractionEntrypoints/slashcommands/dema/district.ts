...
command.setHandler(async (ctx) => {
	...
	const hasRole = ctx.member.roles.cache.find((r) =>
		(districtRoleIds as string[]).includes(r.id),
	);
	if (hasRole)
		throw new CommandError(
			`You have already been assigned to ${hasRole.name.toUpperCase()}.`,
		);
	...
	const secondQuestion = new ActionRowBuilder<TextInputBuilder>().setComponents(
		new TextInputBuilder()
			...
			.setPlaceholder(
				"WE'RE HAPPY TO HAVE YOU. PLEASE ENTER YOUR REASON FOR RELOCATING TO DEMA.",
			)
...
const genModalSubmitId = command.addInteractionListener(
	...
	async (ctx) => {
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				"Thank you for choosing the Dema relocation program. We are now processing your application.",
			)
			.setColor(Colors.Red)
		...
		const bishopName = Object.entries(roles.districts)
			.find((x) => x[1] === assignedRoleId)?.[0]
			?.toUpperCase();
		...
		const staffEmbed = new EmbedBuilder()
			...
			.setDescription(
				`${ctx.member} has been assigned to ${role.name.toUpperCase()}.`,
			)
