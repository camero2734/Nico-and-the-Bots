...
export abstract class ContextMenu<
	T extends keyof TargetTypes,
> extends InteractionEntrypoint<ContextMenuHandler<T>> {
	public commandData: ApplicationCommandData;

	static GenericContextType:
		| MessageContextMenuCommandInteraction
		| UserContextMenuCommandInteraction;
	...
	async _run(ctx: Interaction): Promise<void> {
		if (!ctx.isContextMenuCommand())
			throw new CommandError("Not a context menu");
