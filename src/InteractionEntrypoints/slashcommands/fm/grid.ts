...
const command = new SlashCommand({
	...
	options: [
		...
		},
		{
			name: "relative",
			description:
				"Change the brightness of the album art based on relative playcount",
			required: false,
...
command.setHandler(async (ctx) => {
	...
	const images = await Promise.allSettled(
		collected.map((c) =>
			loadImage(c.image?.replace("/300x300/", imageSize) || ""),
		),
	);
	...
	for (let i = 0; i < collected.length; i++) {
		...
		const image =
			fetchImage.status === "fulfilled" && fetchImage.value
				? fetchImage.value
				: new Image();
