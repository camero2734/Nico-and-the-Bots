...
command.setHandler(async (ctx) => {
	...
	if (
		discordPart !== "discord.com" ||
		channelsPart !== "channels" ||
		guildid !== guildID
	)
		throw new CommandError("Invalid url given");

	const channel = ctx.channel.guild.channels.cache.get(
		channelid,
	) as TextChannel;
	...
	if (msg.author.id !== ctx.user.id)
		throw new CommandError("This is not your message");

	const dbUser = await queries.findOrCreateUser(ctx.member.id);

	if (dbUser.joinedAt < msg.createdAt)
		throw new CommandError("Your join date is already before this message.");
	...
	const embed = new EmbedBuilder()
		...
		.addFields([
			{ name: "Reference message", value: msg.content || "*No content*" },
		])
		.addFields([
			{ name: "New join date", value: F.discordTimestamp(msg.createdAt) },
		])
		.addFields([
			{
				name: "Member num change",
				value: `${ordinal(oldJoinedNum)} → ${ordinal(newJoinedNum)}`,
			},
		]);
