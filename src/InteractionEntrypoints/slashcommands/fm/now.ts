...
command.setHandler(async (ctx) => {
	...
	const username = await getFMUsername(
		ctx.opts.username,
		ctx.opts.user,
		ctx.member,
	);
	...
	if (!total) {
		throw new CommandError(
			`Unable to get your recent tracks. Are you sure your username ${username} is correct?`,
		);
	}
	...
	function us_pl(us_in: string) {
		...
		return us_in.replace(
			"last.fm/music",
			`last.fm/user/${username}/library/music`,
		);
	}
	...
	const trackUrl =
		spotifyResults?.body.tracks?.items?.[0]?.external_urls.spotify;
	if (trackUrl) {
	...
	const geniusResult = await GeniusClient.getSong(
		[trackName, artistName].join(" "),
	);
	if (geniusResult) {
...
const genStarId = command.addInteractionListener(
	"fmStarInt",
	["fmStarId"],
	async (ctx) => {
