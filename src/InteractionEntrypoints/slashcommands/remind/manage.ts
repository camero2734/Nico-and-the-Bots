...
// Main list
async function generateReminderList(member: GuildMember): Promise<[EmbedBuilder] | [EmbedBuilder, ActionRowBuilder]> {
  ...
  if (reminders.length < 1) {
    return [
      new EmbedBuilder({
        description: "You don't have any reminders! Create one using `/remind new`",
      }),
    ];
  }
