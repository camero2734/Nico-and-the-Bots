...
export const GenSongBtnId = msgInt.addInteractionListener(
	"shopSongsBtn",
	[],
	async (ctx) => {
...
// Main Menu
const genMainMenuId = msgInt.addInteractionListener(
	"songSongsMenu",
	[],
	async (ctx) => {
...
// Category submenu
const genSubmenuId = msgInt.addInteractionListener(
	"songSongsSubmenu",
	["categoryId"],
	async (ctx, args) => {
		...
		const [name, category] =
			Object.entries(categories).find(
				([_id, data]) => data.id === args.categoryId,
			) || [];
		...
		const rawComponents = category.data.roles.map((role) => {
			...
			const builder = new ButtonBuilder()
				...
				.setCustomId(
					!ownsRole
						? genItemId({ itemId: role.id, action: `${ActionTypes.View}` })
						: NULL_CUSTOM_ID(),
				);
...
const genItemId = msgInt.addInteractionListener(
	"songSongsItem",
	["itemId", "action"],
	async (ctx, args) => {
		...
		const [categoryName, category] =
			Object.entries(categories).find(([_, category]) =>
				category.data.roles.some((r) => r.id === args.itemId),
			) || [];
		...
		if (dbUser.songRoles.some((r) => r.roleId === role.id))
			throw new CommandError("You already have this role!");
		...
		const shopImage = contraband
			? "https://i.imgur.com/eQEaugK.png"
			: "https://i.redd.it/wd53naq96lr61.png";
		...
		if (actionType === ActionTypes.View) {
			...
			if (ctx.member.roles.cache.some((r) => r.id === role.id))
				throw new CommandError("You already have this role!");
			...
			await prisma.$transaction([
				prisma.songRole.create({
					data: {
						userId: ctx.user.id,
						roleId: role.id /** amountPaid: category.data.credits TODO */,
					},
				}),
				prisma.user.update({
					where: { id: dbUser.id },
				...
				.addFields([
					{
						...
						value:
							"To actually apply this role, simply use the `/roles songs` command. You may only have one song role applied at a time (but you can own as many as you want).",
					},
				]);
			let sent = false;
			try {
...
async function generateMainMenuEmbed(
	member: GuildMember,
