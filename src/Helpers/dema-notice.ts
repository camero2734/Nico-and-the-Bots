...
type ViolationDataGenerator = (
	bishop: BishopType,
	inData: string,
) => { name: string; found: string; reason: string };
const violationData: Record<ViolationType, ViolationDataGenerator> = {
	PossessionOfContraband: (_bishop, roleName) => ({
		...
		found:
			"in possession of regulated materials that have been outlawed by the Dema Council",
...
export async function sendViolationNotice(
	...
	const bishop =
		issuingBishop || F.randomValueInArray(Object.values(BishopType));
	...
	const { name, found, reason } = violationData[violation](
		bishop,
		options.data || "",
	);
	...
	fillParagraph(
		cctx,
		`NOTICE: ${body}.\n\nWe have people on the way. We want you home safe.`,
		mx,
		540,
		width - 2 * mx,
	); // prettier-ignore
	...
	const transmissionEmbed = new EmbedBuilder().setDescription(
		"RECEIVING TRANSMISSION FROM DEMA COUNCIL...",
	);
	...
	for (let i = 0; i < 5; i++) {
		...
		transmissionEmbed.setDescription(
			description.trim() + F.randomizeLetters(".      " as string, 0.1),
		);
		await F.wait(1000);
