...
export type SlashCommandData<T extends CommandOptions = ApplicationDataType[]> = Omit<
  ApplicationCommandData,
  "options" | "name"
> & {
  description: string;
...
type ToObject<T extends DeepReadonly<CommandOptions[number]>> = AsArray<T> extends readonly [
  ...
  ? Key extends PropertyKey
    ? {
        [P in Key]:
          | (Choices extends DeepReadonly<ApplicationCommandOptionChoiceData[]> // If choices provided...
