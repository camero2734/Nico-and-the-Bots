...
command.setHandler(async (ctx) => {
	...
	if (ctx.opts.num === 1) {
		...
		const membersInServerIds = new Set(
			(await ctx.guild.members.fetch()).keys(),
		);
		...
		const membersMarkedInactive =
			membersInServerIds.intersection(inactiveUsers);
		...
		const membersInDatabaseSet = new Set(
			membersInDatabaseWithoutBfxBadge.map((u) => u.id),
		);
