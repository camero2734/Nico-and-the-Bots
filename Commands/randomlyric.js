module.exports = {
    execute: async function(msg) {
        ...
        try {
            ...
            let song;
            let attempts = 0;
            while (stanzas.length === 0 || (stanzas.length === 1 && stanzas[0] === "")) {
                if (attempts >= 10) return msg.channel.send("Unable to get lyrics from Genius");
                await new Promise(next => setTimeout(next, attempts++ > 0 ? 100 : 0));
                song = await genius.song((await genius.search(query))[0].id, { fetchLyrics: true });
                stanzas = song.lyrics.split("\n\n");
            }
