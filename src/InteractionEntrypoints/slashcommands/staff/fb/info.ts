...
command.setHandler(async (ctx) => {
	...
	const unsubmitted = await prisma.firebreatherApplication.count({
	...
	const denied = await prisma.firebreatherApplication.count({
		where: { decidedAt: { not: null }, approved: false },
	});
		...
		.addFields([
			{
				...
				value: `${unsubmitted} application${F.plural(unsubmitted)}`,
			},
		])
		.addFields([
			{
				name: "Needs decision",
				value: `${awaiting} application${F.plural(awaiting)}`,
			},
		])
		.addFields([
			{
				name: "Approved",
				value: `${approved} application${F.plural(approved)}`,
			},
		])
		.addFields([
			{ name: "Denied", value: `${denied} application${F.plural(denied)}` },
		]);
