...
client.on("ready", async () => {
	...
	const botChan = (await guild.channels.fetch(
		channelIDs.bottest,
...
async function getReplyInteractionId(msg: Discord.Message) {
	if (
		!msg.reference ||
		msg.reference.type !== Discord.MessageReferenceType.Default
	)
		return;
	...
	for (const component of repliedTo.components) {
		...
		for (const btn of component.components) {
			...
			if (
				!btn.customId?.startsWith("##!!RL") ||
				!btn.customId?.endsWith("RL!!##")
			)
				continue;

			const replyId = btn.customId
				.replace(/^##!!RL/, "")
				.replace(/RL!!##$/, "");
...
client.on("guildMemberUpdate", async (oldMem, newMem) => {
	if (
		!oldMem.roles.cache.has(roles.deatheaters) &&
		newMem.roles.cache.has(roles.deatheaters)
	) {
...
client.on("messageReactionAdd", async (reaction, user) => {
	...
	for (const [name, handler] of ReactionHandlers.entries()) {
		const wasSuccessful = await handler(
			fullReaction,
			fullUser,
			async (promise) => {
				try {
...
client.on("interactionCreate", async (interaction) => {
	...
	if (interaction.isChatInputCommand()) {
		const commandIdentifier =
			SlashCommand.getIdentifierFromInteraction(interaction);
		const command = SlashCommands.get(commandIdentifier);
		if (!command)
			return console.log(`Failed to find command ${commandIdentifier}`);
		...
		if (!contextMenu)
			return console.log(`Failed to find context menu ${ctxMenuName}`);
