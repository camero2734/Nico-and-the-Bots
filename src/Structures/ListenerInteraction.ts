...
export type ListenerInteraction = (MessageComponentInteraction | ModalSubmitInteraction) &
  RequiredDiscordValues & { message: Message; log: BotLogger };
...
export const createInteractionListener = <T extends Readonly<string[]>>(
...
): [string, InteractionListener<T>, (args: ReturnType<CustomIDPattern<T>["toDict"]>) => string] => {
  ...
  const customIdGenerator = (args: ReturnType<CustomIDPattern<T>["toDict"]>): string => {
    ...
    const ordered = F.entries(args).sort(([key1], [key2]) => pattern.args.indexOf(key1) - pattern.args.indexOf(key2));
