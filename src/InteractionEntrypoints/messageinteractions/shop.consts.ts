...
export class ShopCategory {
	...
	constructor(
		roles: Role[],
		opts: Partial<{ level: number; credits: number; DE: boolean }>,
	) {
	...
	unlockedFor(member: GuildMember, dbUser: User) {
		const meetsDE = this.requiresDE
			? member.roles.cache.has(roles.deatheaters)
			: true;
...
export function getColorRoleCategories(roleManager: RoleManager) {
	...
	return {
		"The Scaled Back Collection": {
			...
			data: tier1,
			description:
				"Looking for basic colors? The Scaled Back Collection has you covered.",
		},
		"Violets of Vetomo": {
			id: "Violet",
			data: tier2,
			description:
				"Ready to branch out into more vibrant colors? Violets of Vetomo is here to help.",
		},
		...
		"Keons' Neons": {
			...
			data: tier4,
			description:
				"These colors are to die for! Handpicked by Keons himself, before... well, you know.",
		},
		"DEMA's Dreamers": {
			id: "DEMA",
			data: tier5,
...
export function getSongRoleCategories(roleManager: RoleManager) {
	...
	const regionalAtBest = new ShopCategory(
		tierToRoles(songRoles.regionalAtBest),
		{ credits: 12_500, level: 15 },
	);
