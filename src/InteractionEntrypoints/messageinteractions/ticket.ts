...
const genBtnId = ticketInteraction.addInteractionListener(
	"ticketOpenModal",
	[],
	async (ctx) => {
...
const genModalId = ticketInteraction.addInteractionListener(
	"ticketSubmitModal",
	[],
	async (ctx) => {
		...
		await ctx.editReply(
			`Ticket submitted! A staff member will be with you shortly in <#${thread.id}>`,
		);
	},
);
