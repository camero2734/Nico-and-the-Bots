...
command.setHandler(async (ctx) => {
	...
	const remainingTime =
		waitTime.lastTaken.getTime() +
		VerifiedQuizConsts.DELAY_BETWEEN_TAKING -
		Date.now();
	if (remainingTime > 0 && !ctx.member.roles.cache.has(roles.staff)) {
		const hours = (remainingTime / (1000 * 60 * 60)).toFixed(2);
		throw new CommandError(
			`You must wait ${hours} hours before applying again.`,
		);
	}
	...
	const initialEmbed = new EmbedBuilder()
		.setTitle(
			"<:cliffordpopcorn:893992063677366295> Vulture Verification Validator",
		)
		.setDescription(
			[
				"# Verified Quiz",
		...
		.addFields([
			...
			},
			{
				...
				value:
					"Once granted, access to the channel is conditional on following the rules. If you break the rules, you will be removed from the channel and will not be able to reapply for a set period of time.",
			},
		]);
	...
	try {
		...
		throw new CommandError(
			"You must have server DMs enabled to use this command",
		);
	}
	...
	const actionRow = new ActionRowBuilder<ButtonBuilder>().setComponents([
		new ButtonBuilder()
			...
			.setCustomId(genModalId({ seed36 })),
		new ButtonBuilder()
			.setLabel("Cancel")
			.setStyle(ButtonStyle.Danger)
			.setCustomId(genCancelId({})),
	]);
...
const genCancelId = command.addInteractionListener(
	"verifcancel",
	[],
	async (ctx) => {
		...
		await ctx.editReply({
			embeds: [
				new EmbedBuilder().setDescription(
					"Okay, you may restart the quiz at any time.",
				),
			],
			components: [],
		});
...
const genModalId = command.addInteractionListener(
	"verifmodal",
	["seed36"],
	async (ctx, args) => {
		if (!ctx.isButton()) return;
		...
		if (rawActionRow.type !== ComponentType.ActionRow)
			throw new Error("Invalid action row");

		const dmActionRow = new ActionRowBuilder<ButtonBuilder>(
			rawActionRow.toJSON(),
		);
		const beginBtn = dmActionRow.components.find(
			(c) => getLabel(c.data) === "Begin" || getLabel(c.data) === "Reopen",
		);
		const cancelBtn = dmActionRow.components.find(
			(c) => getLabel(c.data) === "Cancel",
		);
		...
		embed.setFooter({
			text: "You have started the quiz. Click 'Reopen' to continue.",
		});
...
const genModalSubmitId = command.addInteractionListener(
	"verifmodaldone",
	["seed36"],
	async (ctx, args) => {
		...
		const staffChan = await guild?.channels.fetch(
			channelIDs.verifiedapplications,
		);
		if (staffChan?.isTextBased())
			await staffChan.send({ embeds: [staffEmbed] });

		// Generate initial variables
		const questionList = F.shuffle(QuizQuestions).slice(
			0,
			VerifiedQuizConsts.NUM_QUESTIONS,
		);
		const answerEncoder = new PreviousAnswersEncoder(questionList);
		const questionIDs = QuestionIDEncoder.encode(questionList);
		const [quizEmbed, quizComponents] = await generateEmbedAndButtons(
			-1,
			questionList,
			answerEncoder,
			questionIDs,
...
const genVeriquizId = command.addInteractionListener(
	"veriquiz",
	["currentID", "questionIDs", "previousAnswers"],
	async (ctx, args) => {
		...
		if (!member)
			return console.log(
				`[Verified Quiz] Member does not exist: ${ctx.user.id}`,
			);
		...
		const answerEncode = new PreviousAnswersEncoder(questionList).fromString(
			previousAnswers,
		);
...
> {
	...
	if (newIndex === numQs)
		return sendFinalEmbed(questionList, answerEncode, member);
	...
	const embed = new EmbedBuilder()
		...
		.setFooter({
			text: "Select the correct answer by selecting an option below",
		});
...
> {
	...
	staffEmbed
		.setTitle(
			`${passed ? "Passed" : "Failed"}: ${correct}/${questionList.length} correct`,
		)
		.setColor(passed ? 0x88ff88 : 0xff8888);
	const staffChan = member.guild.channels.cache.get(
		channelIDs.verifiedapplications,
	) as TextChannel;
	await staffChan.send({ embeds: [staffEmbed] });
	...
	embed.addFields([
		{
			name: "Be On Topic",
			value: "All discussion must be related to theories and lore of the band.",
		},
		{
			name: "Be Informed",
			value: `Make sure you're up to date by reading ${channelMention(channelIDs.loreupdates)} and ${channelMention(channelIDs.confirmedfakestuff)}`,
		},
		{
			name: "Be Constructive",
			value: "All discussion should be constructive and respectful of others.",
		},
		{
			name: "No General Discussion",
			value: `General discussion should be kept to the appropriate channels, like ${channelMention(channelIDs.hometown)} or ${channelMention(channelIDs.slowtown)}. Memes belong in  ${channelMention(channelIDs.eramemes)}.`,
		},
		{
			name: "No Spoilers",
			value:
				"Do not spoil any leaked or unofficial content for others. This does not include official teasers, like dmaorg.info.",
		},
	]);
	...
	const questions = [
		[
			"I'll be on topic in the channel",
			"I can discuss anything I want",
			"I might send some memes",
		],
		[
			"I'll keep up to date before posting",
			"I don't need to know anything",
			"I can make up my own lore",
		],
		[
			"I will respect other's theories",
			"I will dismiss anyone who doesn't agree with me",
			"I will dunk on bad theories",
		],
		[
			"I won't talk about random things",
			"I'll break out into a random topic",
			"I'll send so many jokes",
		],
		[
			"I won't discuss leaked content",
			"I'll share leaked content",
			"I am become leaker, destroyer of surprises",
		],
	]
...
const genPartThreeBtnId = command.addInteractionListener(
	"verifopenmodalagree",
	["idx"],
	async (ctx, args) => {
		if (!ctx.isStringSelectMenu()) return;
		...
		const allCorrect = newActionRows.every((row) => {
			if ("type" in row && row.type !== ComponentType.ActionRow)
				throw new CommandError("Invalid action row");

			const selectMenu = row.components[0].toJSON();
			return (
				selectMenu.type === ComponentType.StringSelect && selectMenu.disabled
			);
		});
...
function generatePartOne(seed: number) {
	...
	const ascii = asciiDecoded
		.split("")
		.map((c) => c.charCodeAt(0).toString(randomBase))
		.join(" ");
	...
	const order = faker.helpers.shuffle(Object.keys(options)) as Array<
