...
export class FooterEncoder<T extends Record<keyof T, string | number>> {
	...
	encode(data: T): string {
		const orderedEntries = Object.entries(data).sort((ent1, ent2) =>
			ent1[0].localeCompare(ent2[0]),
		);
	...
	decode(text: string): T {
		...
		const decrypted = Buffer.concat([decryptedBuf, decipher.final()]).toString(
			"utf-8",
		);

		const orderedKeys = Object.keys(this.exampleData).sort((key1, key2) =>
