...
export const CONCERT_URL =
	"https://rest.bandsintown.com/V3.1/artists/twenty%20one%20pilots/events/?app_id=js_127.0.0.1";
...
export class ConcertChannel {
	...
	get venueId() {
		...
		const s = (str: string) =>
			str
				...
				.replace(/[^A-z0-9]/g, " ")
				.split(/ +/);
		return [s(this.name), s(venue.city)]
			.flat()
			.filter((a) => a)
			.join("-");
	}
	...
	get datesFormatted() {
		return this.concerts
			.map((c) => format(new Date(c.datetime), "d MMMM yyyy"))
			.join(", ");
	}
	...
	async threadTags(forumChannel: ForumChannel) {
		...
		if (hasMultipleDates) {
			const tag = forumChannel.availableTags.find((t) =>
				t.name.toLowerCase().includes("multiple shows"),
			);
		...
		if (continent) {
			const tag = forumChannel.availableTags.find((t) =>
				t.name.toLowerCase().includes(continent),
			);
			if (tag) tags.push(tag);
		}
