module.exports = {
    execute: async function(msg) {
        ...
        await msg.channel.embed("You muted yourself for " + mins + " minutes. If you want the timeout to end early... figure it out...");

        await connection.getRepository(Item).createQueryBuilder().delete().where("type = :type", { type: "Timeout" }).andWhere("id = :id", { id: msg.author.id }).execute();
        let newTimeout = new Item({ id: msg.author.id, title: "Self", type: "Timeout", time })
