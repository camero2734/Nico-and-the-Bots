...
command.setHandler(async (ctx) => {
	...
	const channelPermissionsBackup =
		await prisma.channelPermissionsBackup.findFirst({
			where: { channelId: ctx.channel.id },
		});

	if (channelPermissionsBackup) {
		const timestamp = F.discordTimestamp(
			channelPermissionsBackup.createdAt,
			"relative",
		);
	...
	const permissions = [...ctx.channel.permissionOverwrites.cache.values()].map(
	...
	await ctx.channel.permissionOverwrites.edit(roles.banditos, {
		SendMessages: false,
	});
	await ctx.channel.permissionOverwrites.edit(roles.staff, {
		SendMessages: true,
	});
		...
		.addFields([
			{
				...
				value:
					"You may unlock this channel with the `/staff lockdown end` slash command.",
			},
		]);
