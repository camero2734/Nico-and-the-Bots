...
command.setHandler(async (ctx) => {
	...
	if (!member)
		throw new CommandError(
			"Could not find this member. They may have already been banned or left.",
		);
	...
	const bannedEmbed = new EmbedBuilder()
		...
		.setDescription(
			"You have been banned from the twenty one pilots Discord server",
		)
		...
		bannedEmbed.addFields([
			{
				name: "Appeal",
				value:
					"You may appeal your ban by visiting:\ndiscordclique.com/appeals",
			},
		]);
	}
	...
	await ctx.send({
		embeds: [
			new EmbedBuilder({
				description: `${member.toString()} was banned.`,
			}).toJSON(),
		],
	});
...
async function sendToBanLog(
	ctx: typeof command.ContextType,
	bannedEmbed: EmbedBuilder,
) {
	const banLogChannel = (await ctx.guild.channels.fetch(
		channelIDs.banlog,
	)) as TextChannel;
