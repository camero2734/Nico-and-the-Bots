module.exports = {
    execute: async function (msg) {
        ...
        if (msg.content.split(" ").slice(2).length > 0) {
            ...
            let regex = /(?<rule>[BDSNObdsno]) {0,1}(?<severity>\d{1,2})+$|(?<severity2>\d{1,2})+ {0,1}(?<rule2>[BDSNObdsno])$/
        ...
        confirmationEmbed.addField("Rule Broken", rules.find(_r => _r.startsWith(rule.toUpperCase())));
        ...
        if (confirmation_msg.content.toLowerCase().indexOf("yes") !== -1) {
            ...
            let warnData = { edited: false, given: msg.author.id, channel: msg.channel.id, rule: rules.find(_r => _r.startsWith(rule.toUpperCase())), severity: parseInt(severity), content: explanation };
