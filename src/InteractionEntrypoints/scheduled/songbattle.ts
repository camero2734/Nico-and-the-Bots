...
async function createMessageComponents(details: SongBattleDetails): Promise<MessageEditOptions> {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields([
            { name: `${song1.song.name}${wins1}`, value: `${emoji1} ${italic(song1.album.name)}\n📽️ [YouTube Video](${song1.song.yt})`, inline: true },
            { name: `${song2.song.name}${wins2}`, value: `${emoji2} ${italic(song2.album.name)}\n📽️ [YouTube Video](${song2.song.yt})`, inline: true },
        ])
